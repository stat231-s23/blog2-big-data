...
data <- raw_data %>%
  ...
  mutate(complaint = case_when(
    ...
    abbr == "alle" | abbr == "hive" | abbr == "mild" | abbr == "mino" | abbr == "rash"
    ~ "allergic reaction",
    abbr == "anap" ~ "anaphylaxsis",
    abbr == "beha" | abbr == "ment" | abbr == "pani" ~ "mental health",
    abbr == "burn" ~ "burn",
    abbr == "card" ~ "cardiovascular emergency",
    abbr == "ches" | abbr == "non" ~ "chest pain",
    abbr == "dizz" ~ "dizziness",
    abbr == "envi" | abbr == "chem" ~ "environmental hazard",
    abbr == "etoh" ~ "alcohol and/or drugs",
    abbr == "eye" ~ "eye pain", #check this again (rows 588-594)
    abbr == "fall" ~ "fall",
    abbr == "flu-" ~ "flu-like symptoms",
    abbr == "gast" | abbr == "naus" | abbr == "stom" 
    ~ "gastrointestinal",
    complaint == "head injury" ~ "head injury", #check this again (rows 808-820)
    complaint == "headache" ~ "headache", #check this again(rows 821-826)
    abbr == "heat" | abbr == "hot" ~ "heat exhaustion",
    abbr == "hemo" | abbr == "lace" ~ "laceration/hemorrhage",
    abbr == "hypo" ~ "hypoglycemia",
    abbr == "moto" ~ "motor vehicle accident",
    abbr == "musc" ~ "musculoskeletal injury",
    abbr == "over" | abbr == "took" ~ "non-alcoholic overdose",
    abbr == "resp" | abbr == "tons" ~ "respiratory distress",
    abbr == "seiz" ~ "seizure",
    abbr == "soft" ~ "soft tissue injury",
    abbr == "stro" ~ "stroke",
    abbr == "sync" ~ "syncope",
    abbr == "tick" ~ "tick bite",
    TRUE ~ "other"
...
visual4data <- data %>% 
  mutate(
    ...
    time_of_day = case_when(
      time_of_call >= "00:00:00" & time_of_call < "12:00:00" ~ "Late night"
    )
  )
```
