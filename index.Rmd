...
#WRANGLING FOR TABLE AND BAR CHART
...
case_summary <- init_summary_data %>%
  ...
  arrange(desc(N)) %>%
  head(10) %>%
  #datatable(colnames = c("Complaint", "Count"),
  #          filter = 'top',
  #          options = list(pageLength = 10, autoWidth = 10))# %>%
  kbl(caption = "Initial summary of case distribution") %>%
  kable_classic(full_width = F, html_font = "Cambria")

plot1 <- ggplot(case_summary1, aes(x = reorder(abbr, -N), y = N)) + geom_bar(stat = "identity") +
  labs(
...
```{r init wrangling, message=FALSE, echo = FALSE}

```

```{r init_summary-wrangling, message=FALSE, echo=FALSE}
...
For our initial data collection and preparation phase, *getting* the data set was the easiest part. As mentioned previously, Austin is a senior member of ACEMS --- and as all members are responsible for updating the emergency response common data --- he was easily able to access the spreadsheet. After ensuring that the data were extensive and had potential for interesting interpretation, we set out on acquiring permission. Although the data omitted any personal information which could identify a victim, we wanted to be certain continuing was ethical. And luckily, after talking to Professor Correia and Austin's supervisor, we were good to go!
...
From the map, it's clear that the vast majority of ACEMS calls occur on the first-year quad. With over 150 more calls than Valentine quad, the second highest location, this is an astonishing amount. We question whether this statistic is simply a function of the first-year quad having the highest density of students, first-years being more reckless/irresponsible, or some combination.

Otherwise, though, the trends are about as we expected. The areas near typical "party dorms" are associated with greater amounts of calls. The gym and athletic fields also have relatively high numbers of calls, which makes sense given the potential for fatigue and exhaustion.
...
ggplot(data = visual4day, aes(x = time_of_day, y = count, fill = call_type)) +
  geom_bar(stat = 'identity') +
   scale_fill_manual(values = c("red", "blue"), name = "Type of emergency",
...
Moving on to long-term trends, we plotted the amount of calls from Fall 2016 to Spring 2023. One interesting trend is that the Fall semesters appear to be associated with higher amounts of cases. This might be explained by temperature differences. Given that Fall is generally warmer than Spring, there might be a greater amount of alcohol-related emergencies.


```{r, message=FALSE, echo=FALSE}
visual3data <- data %>% 
    #filter(location == "first-year quad") %>%
  mutate(
...
ggplot(visual3data, aes(x = semester, y = count, group = call_type, color = call_type)) +
  ...
  geom_line() +
  labs(
    title = "Distribution of calls to FYQ throughout the semester",
    x = "Semester",
    y = "Number of calls"
  ) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  scale_color_manual(values = c("blue", "black", "gray"), name = "Type of emergency",
                    labels = c("Alcohol calls only", "All calls", "First-year quad only")) +
  scale_y_continuous(breaks = c(0, 20, 40, 60, 80, 100, 120, 140, 160), 
                     labels = c("0", "20", "40", "60", "80", "100", "120", "140", "160"))
```
Now, we will analyze trends on response escalation based on type of emergency.

# Central Question 3: Escalation of ACEMS Calls
...
As discussed previously, we significantly consolidated the number of emergency complaint labels in our data set. With our final list, we hoped to 
...
final_case_summary <- data %>%
 dplyr::group_by(complaint) %>%
  ...
  #          filter = 'top',
  #          options = list(pageLength = 10, autoWidth = 10))# %>%
  kbl(caption = "Summary of case distribution") %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
<div style= "float:right;position: relative; top: -1px; right: -20px">
```{r}
...
```{r}
ggplot(visual2data, aes(x = complaint, y = percentage, fill = type_of_care)) + 
  geom_bar(stat = "identity") +
  labs(
